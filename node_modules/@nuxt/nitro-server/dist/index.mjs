...
async function bundle(nuxt) {
	...
	const foldRouteRuleKey = (route) => caseSensitiveRouteRules || typeof route !== "string" ? route : route.toLowerCase();
	function getRouteRulesRouter(fold) {
		const routeRulesRouter = createRouter();
		if (nuxt._nitro) {
			const foldedKeys = /* @__PURE__ */ new Map();
			for (const [route, rules] of Object.entries(nuxt._nitro.options.routeRules)) {
				...
				if (fold) {
					...
					if (existing !== void 0 && existing !== route && !caseSensitiveRouteRules) logger.warn(`Route rules for \`${existing}\` and \`${route}\` resolve to the same path when matched case-insensitively; \`${route}\` takes precedence. Disambiguate the keys or set \`router.options.sensitive: true\`.`);
			...
			nitro.hooks.hook("rollup:before", async (nitro) => {
				...
				if (nitro._prerenderedRoutes?.length) {
					...
					for (const route of nitro._prerenderedRoutes) if (!route.error && route.route.endsWith(payloadSuffix)) {
						...
						if (!defu({}, ...findAllRoutes(routeRulesMatcher, void 0, foldRouteRuleKey(url)).reverse()).prerender) prerenderedRoutes.add(url);
					}
				}
				const manifest = {
					id: buildId,
